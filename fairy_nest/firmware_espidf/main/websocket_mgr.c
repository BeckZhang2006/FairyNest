...
static void handle_command(const char *json_str)
{
    ...
    // Handle server message types first
    if (type) {
        if (strcmp(type, "voice_result") == 0) {
            cJSON *text = cJSON_GetObjectItem(root, "text");
            if (cJSON_IsString(text)) {
                ESP_LOGI(TAG, "Voice result: %s", text->valuestring);
                // Forward STT result to voice FSM
                fairy_msg_t msg = {.type = MSG_VOICE_STT_RESULT};
                strncpy(msg.param_s, text->valuestring, sizeof(msg.param_s) - 1);
                msg.param_s[sizeof(msg.param_s) - 1] = '\0';
            ...
            fairy_msg_t msg = {.type = MSG_VOICE_PLAY_TTS};
            xQueueSend(g_msg_queue, &msg, 0);
            cJSON_Delete(root);
            return;
        }
            ...
            fairy_msg_t msg = {.type = MSG_VOICE_TTS_END};
            xQueueSend(g_msg_queue, &msg, 0);
            cJSON_Delete(root);
            return;
        }
    ...
    if (strcmp(command, "set_light") == 0) {
        ...
        if (cJSON_IsString(text)) {
            ...
            // Legacy TTS trigger, treat as tts_start
            fairy_msg_t msg = {.type = MSG_VOICE_PLAY_TTS};
            xQueueSend(g_msg_queue, &msg, 0);
        }
    }
    ...
    switch (event_id) {
        ...
        case WEBSOCKET_EVENT_DATA:
            if (data->op_code == WS_TRANSPORT_OPCODES_TEXT &&
                ...
                // TTS audio data - forward to I2S TX for playback
                if (data->data_len > 0) {
                    i2s_audio_play_pcm((const uint8_t *)data->data_ptr, data->data_len);
                }
            }
            break;

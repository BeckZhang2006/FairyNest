...
void i2s_rx_task(void *pvParameters)
{
    ...
    for (;;) {
        ...
        if (!s_recording) {
            ...
            // Wake word detection trigger (simplified energy trigger)
...
void i2s_tx_task(void *pvParameters)
{
    ...
    for (;;) {
        ...
        // Check for messages from queue (TTS playback)
        fairy_msg_t msg;
        if (xQueueReceive(g_msg_queue, &msg, 0) == pdTRUE) {
            if (msg.type == MSG_VOICE_PLAY_TTS) {
                // TTS data would be handled here
                // For now, just acknowledge
                ESP_LOGD(TAG, "TTS playback requested");
            }
        }
    }
}

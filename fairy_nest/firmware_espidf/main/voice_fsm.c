    ...
    for (;;) {
        ...
        switch (s_state) {
            ...
            case VOICE_STATE_RECORDING: {
                // Monitor recording duration and silence
                ...
                // Check silence timeout (simplified - full VAD in i2s_rx_task)
                // Here we rely on the recording completion from energy detection
                break;
            }

            case VOICE_STATE_PROCESSING: {
                // In production: receive STT result from cloud
                // For now: simulate with a timeout and return to idle
                if (now_ms - s_state_enter_ms > 2000) {
                    ESP_LOGI(TAG, "Processing timeout, returning to idle");
            ...
            case VOICE_STATE_PLAYING: {
                ...
                if (now_ms - s_state_enter_ms > 3000) {
                    transition_to(VOICE_STATE_IDLE);
                }
                break;
            }

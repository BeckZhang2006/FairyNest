 ...
 *
 * Reference: RuView (https://github.com/ruvnet/RuView)
 */
...
void csi_rx_callback(void *ctx, wifi_csi_info_t *data)
{
    ...
    // Each subcarrier: [imaginary_1byte, real_1byte]
    float amplitudes[CSI_SAMPLE_COUNT];
    int sc_count = 0;

    for (int i = 0; i < len - 1 && sc_count < CSI_SAMPLE_COUNT; i += 2) {
        int8_t imag = buf[i];
        int8_t real = buf[i + 1];
        amplitudes[sc_count] = sqrtf((float)(real * real) + (float)(imag * imag));
        sc_count++;
    }

    if (sc_count < 2) return;

    // Hampel filter (simplified inline for ISR safety)
    for (int i = 3; i < sc_count - 3; i++) {
        float window[7];
        for (int j = 0; j < 7; j++) window[j] = amplitudes[i - 3 + j];
        ...
        float median = window[3];

        // Check if outlier
        if (fabsf(amplitudes[i] - median) > median * 1.5f) {
            amplitudes[i] = median;
        }
    }
    ...
    // Store variance (atomic-ish for single float)
...
void wifi_csi_task(void *pvParameters)
{
    ...
    for (;;) {
        ...
        if (new_presence != PRESENCE_EMPTY) {
            s_last_presence_ms = xTaskGetTickCount() * portTICK_PERIOD_MS;
        }
